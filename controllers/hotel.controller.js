...
function createPDF(req,res){
    ...
    Hotel.findById(hotelID, (err, hotel)=>{
        if(err){
            ...
            doc.pipe(fs.createWriteStream('./pdf/hotelExample.pdf'));
            
            doc.fontSize(25).text(hotel.name, {
                align: 'center',
            ...
            var qualification = hotel.qualification;
            var price = hotel.price;
            var availableStartDate = hotel.availableStartDate;
            var availableEndDate = hotel.availableEndDate;

            doc.fontSize(10).text('Email: ' + email, {
                columns: 1
            });

            doc.fontSize(10).text('Location: ' + location, {
                columns: 1
            });
            ...
            doc.fontSize(10).text('Quantification: ' + qualification, {
                columns: 1
            });

            doc.fontSize(10).text('Price: ' + price, {
                columns: 1
            });

            doc.fontSize(10).text('Available Start Date: ' + availableStartDate, {
                columns: 1
            });

            doc.fontSize(10).text('Available End Date: ' + availableEndDate, {
                columns: 1
            });
            doc.end();
            ...
            res.status(404).send({ message : 'Empresa indicada no encontrada'});
        }
    })
}
module.exports = {
